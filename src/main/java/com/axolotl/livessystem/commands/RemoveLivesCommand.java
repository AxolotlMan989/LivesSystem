...
public class RemoveLivesCommand implements CommandExecutor {
    ...
    public RemoveLivesCommand(LivesSystem plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length < 2) {
            sender.sendMessage(lm.colorize("&cUsage: /removelives <player> <amount>"));
            return true;
        }
        ...
        lm.removeLives(target.getUniqueId(), amount);
        sender.sendMessage(lm.colorize(plugin.getConfig().getString("messages.lives-removed", "&cRemoved &e%amount% &clives from &e%player%&a.")
